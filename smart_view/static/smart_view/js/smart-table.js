...
class SmartTable {
    ...
    init(tabulator_options, smart_view_options) {
        ...
        if (smart_view_options.current_row_manager) {
            ...
            this.current_record_element = document.querySelector(`#${this.prefix}-smart-table-menu-bar .current-record`);
            this.records_count_element = document.querySelector(`#${this.prefix}-smart-table-menu-bar .records-count`);

            var button = document.querySelector(`#${this.prefix}-smart-table-menu-bar .first-record`);
            if (button) {
            ...
            var button = document.querySelector(`#${this.prefix}-smart-table-menu-bar .previous-record`);
            if (button) {
                button.onclick = function (event) {
            ...
            var button = document.querySelector(`#${this.prefix}-smart-table-menu-bar .next-record`);
            if (button) {
                button.onclick = function (event) {
                    // console.debug("next");
            ...
            var button = document.querySelector(`#${this.prefix}-smart-table-menu-bar .last-record`);
            if (button) {
                button.onclick = function (event) {
                    // console.debug("last");
                    self.current_row.getTable().deselectRow();
        ...
        this.tabulator = new Tabulator(`#${this.prefix}-smart-table-table`, this.tabulator_options);
        ...
        // L'utilisation d'une fonction intermediaire permettra de récupérer self comme l'objet SmartTable
        // 'this' sera l'appelant, c'est à dire le Tabulator (dont le constructeur ne sera pas terminé et donc
        // qui ne sera pas encore référencé par l'objet SmartTable)
        ...
        $(`#${this.prefix}-smart-table-menu-bar .views-buttons-box .icon-button`).click(function(event) {
        ...
        this.filters = $(
            `#${this.prefix}-smart-table-filters input, #${this.prefix}-smart-table-filters select, .smart-table-filter-bar input`
        );
        ...
        $(`#${this.prefix}-smart-table-filters-box .smart-table-filter-box i`).css("cursor", "pointer").click(function(event) {
        ...
        // If this SmartTable has a manager, let's subscribe
    ...
    show_view(view, button) {
        // console.log('id=', this, `${this.prefix}-smart-table-${view}`, button);
        var view_elt = document.getElementById(`${this.prefix}-smart-table-${view}`);
    ...
    add_managed(managed) {
        ...
        this.managed_smarttables.push(managed);
    }
    ...
    form_update(self, data) {
        ...
        for (var i in self.options.form_fields) {
            ...
            var elt = document.getElementById(self.prefix + '-smart-table-form-' + field_name);
    ...
    row_selected(self, row, tabulator) {
        ...
        // Update managed SmartTables
        for (var i = 0; i < self.managed_smarttables.length; i++) {
            if (self.managed_smarttables[i].last_manage_value !== row.getData()[self.managed_smarttables[i].fieldname]) {

                // To avoid useless managment (kind of workaround...)
                self.managed_smarttables[i].last_manage_value = row.getData()[self.managed_smarttables[i].fieldname];
                var managed_smarttable = self.managed_smarttables[i];

                // console.debug("Update managed...", managed_smarttable, row.getData()[managed_smarttable.fieldname]);
                var filter = {};
                filter[managed_smarttable.fieldname] = managed_smarttable.last_manage_value;
                managed_smarttable.smarttable.filters_apply(null, managed_smarttable.smarttable, filter);
            }
        }
    ...
    is_editable(cell, tabulator) {
        ...
        var roles = cell.getData()[this.options.roles_field].split(',');
    ...
    filter_value(smarttable, filter) {
     ...
     */
    filters_apply(event, smarttable, base_filter = {}) {
        // console.debug("filters_apply...", event, smarttable);

        var query = smarttable.base_url;
        var filters_kw = base_filter;
        var filters_full = {};

        smarttable.filters.each(function(filter) {
            var filter_box = $(
                `#${smarttable.prefix}-smart-table-filters-box .smart-table-filter-box[data-field="${this.attributes['name'].value}"]`
            );
            var filter_value = $(this).val();
            var filter_label = $(
                `#${smarttable.prefix}-smart-table-filters label[name="${this.attributes['name'].value}"]`).html();
            ...
            if ($(this).is('select')) {
                if (filter_value !== "{}") {
                    ...
                    filter_box.find('.filter-text').html(filter_label + ' : <b>' + $(
                        `#${smarttable.prefix}-smart-table-filters select[name="` + this.attributes['name']
        ...
        var settings = {};
        settings[smarttable.options.appname + '.tabulator-' + smarttable.prefix + '.filters'] = filters_full;
        smarttable.update_settings(smarttable, settings);
        smarttable.tabulator.setData(query + '?smart_view_prefix=' + smarttable.prefix + '&filters=' + encodeURIComponent(JSON
            .stringify(filters_kw)));
    }

    export_url(smarttable, id) {
        let filters_kw = {};
        smarttable.filters.each(function(filter) {
            let filter_value = smarttable.filter_value(smarttable, $(this));
            console.debug('f=', filter_value);
            filter_value = JSON.parse(filter_value);
            Object.assign(filters_kw, filter_value);
        });
        return smarttable.base_url + "?smart_view_prefix=" + smarttable.prefix + '&filters=' + encodeURIComponent(JSON
    ...
    tool_delete_fmt(self, cell, formatterParams, onRendered) {
        ...
        var roles = cell.getRow().getData()[self.options.roles_field].split(',');
